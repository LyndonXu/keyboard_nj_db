...
{
	CODE_SWITCH1_MAX_VALUE, CODE_SWITCH2_MAX_VALUE, 
	CODE_SWITCH4_MAX_VALUE, CODE_SWITCH3_MAX_VALUE,
};
...
static void CodeSwitchPinInit(void)
{
	...
	/* switch3 */
	GPIO_InitStructure.GPIO_Pin = CODE_SWITCH3_PIN_A;
	GPIO_Init(CODE_SWITCH3_PIN_A_PORT, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = CODE_SWITCH3_PIN_B;
	GPIO_Init(CODE_SWITCH3_PIN_B_PORT, &GPIO_InitStructure);

	/* switch4 */
	GPIO_InitStructure.GPIO_Pin = CODE_SWITCH4_PIN_A;
	GPIO_Init(CODE_SWITCH4_PIN_A_PORT, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = CODE_SWITCH4_PIN_B;
	GPIO_Init(CODE_SWITCH4_PIN_B_PORT, &GPIO_InitStructure);
	...
	/* Connect EXTI for switch 2 */
	GPIO_EXTILineConfig(CODE_SWITCH2_INT_SRC_PORT, CODE_SWITCH2_INT_SRC);

	EXTI_InitStructure.EXTI_Line = CODE_SWITCH2_INT_LINE;
	EXTI_Init(&EXTI_InitStructure);
	
	/* Connect EXTI for switch 3 */
	GPIO_EXTILineConfig(CODE_SWITCH3_INT_SRC_PORT, CODE_SWITCH3_INT_SRC);

	EXTI_InitStructure.EXTI_Line = CODE_SWITCH3_INT_LINE;
	EXTI_Init(&EXTI_InitStructure);

	/* Connect EXTI for switch 4 */
	GPIO_EXTILineConfig(CODE_SWITCH4_INT_SRC_PORT, CODE_SWITCH4_INT_SRC);

	EXTI_InitStructure.EXTI_Line = CODE_SWITCH4_INT_LINE;
	EXTI_Init(&EXTI_InitStructure);
	...
	/* Enable and set switch 2 Interrupt */
	NVIC_InitStructure.NVIC_IRQChannel = CODE_SWITCH2_INT_CHANNEL;
	NVIC_Init(&NVIC_InitStructure);
	
	/* Enable and set switch 3 Interrupt */
	//NVIC_InitStructure.NVIC_IRQChannel = CODE_SWITCH3_INT_CHANNEL;
	//NVIC_Init(&NVIC_InitStructure);
	
	/* Enable and set switch 4 Interrupt */
	//NVIC_InitStructure.NVIC_IRQChannel = CODE_SWITCH4_INT_CHANNEL;
	//NVIC_Init(&NVIC_InitStructure);
}

#if 0
static bool CodeSwitchScan(StCodeSwitchState *pState, u8 u8PinA, u8 u8PinB)
{
	if (u8PinA == u8PinB)
	{
		pState->u16Cnt++;
...
{
	u16 u16PinATmp = CODE_SWITCH2_PIN_A_PORT->IDR;
	u16 u16PinBTmp = CODE_SWITCH2_PIN_B_PORT->IDR;
	...
	if(EXTI_GetITStatus(CODE_SWITCH3_INT_LINE) != RESET)
	{
		bool boIsCW;
		u16 u16PinA, u16PinB;
		...
		u16PinA &= CODE_SWITCH3_PIN_A;
		u16PinB &= CODE_SWITCH3_PIN_B;
...
#if CODE_SWITCH3_REVERSE
		boIsCW = !boIsCW;
#endif

		if (boIsCW)
		{
			s_stCodeSwitch3.u16Cnt++;
		}
		else
		{
			s_stCodeSwitch3.u16Cnt--;
		...
		if (s_stCodeSwitch3.u16Cnt > CODE_SWITCH3_MAX_VALUE)
		{
			if (boIsCW)
			{
				s_stCodeSwitch3.u16Cnt = 0;
			}
			else
			{
				s_stCodeSwitch3.u16Cnt = CODE_SWITCH3_MAX_VALUE;
			}
		}
		EXTI_ClearITPendingBit(CODE_SWITCH3_INT_LINE);
	}
	if(EXTI_GetITStatus(CODE_SWITCH4_INT_LINE) != RESET)
	{
		bool boIsCW;
		u16 u16PinA, u16PinB;
		...
		u16PinA &= CODE_SWITCH4_PIN_A;
		u16PinB &= CODE_SWITCH4_PIN_B;
...
#if CODE_SWITCH4_REVERSE
		boIsCW = !boIsCW;
#endif

		if (boIsCW)
		{
			s_stCodeSwitch4.u16Cnt++;
		}
		else
		{
			s_stCodeSwitch4.u16Cnt--;
		...
		if (s_stCodeSwitch4.u16Cnt > CODE_SWITCH4_MAX_VALUE)
		{
			if (boIsCW)
			{
				s_stCodeSwitch4.u16Cnt = 0;
			}
			else
			{
				s_stCodeSwitch4.u16Cnt = CODE_SWITCH4_MAX_VALUE;
			}
		}
		EXTI_ClearITPendingBit(CODE_SWITCH4_INT_LINE);
	}
...
void CodeSwitchFlush(void)
{
	...
	for (i = 0; i < CODE_SWITCH_MAX; i++)
	{
		if (CodeSwitchGetValueInner(s_stCodeSwitch + i))
		{
			...
			pBuf->emKeyType = _Key_CodeSwitch;
			memcpy(&(pBuf->unKeyMixIn.stCodeSwitchState), s_stCodeSwitch + i, 
