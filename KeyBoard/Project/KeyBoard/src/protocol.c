...
static bool KeyBoardProcess(StKeyMixIn *pKeyIn)
{
	...
	switch (u8KeyValue)
	{
		...
		case _Key_DDR_Play_Pause:
		{
			pBuf[_YNA_Cmd] = 0x47;
			pBuf[_YNA_Data1] |= 0x10;
			pBuf[_YNA_Data2] = 0x02 + g_boIsDDRPlay;
			break;
		}
		case _Key_DDR_Front:
		case _Key_DDR_Cycle:
		...
		case _Key_Trick_Shortcut_2:
		case _Key_Trick_Shortcut_3:
		case _Key_Trick_Shortcut_4:
		case _Key_Trick_Shortcut_5:
		case _Key_Trick_Shortcut_6:
		case _Key_Trick_Shortcut_7:
		case _Key_Trick_Shortcut_8:
		{
			pBuf[_YNA_Cmd] = 0x48;
		...
		{
			pBuf[_YNA_Cmd] = 0x47;
			pBuf[_YNA_Data2] = 0x05 + g_boIsEdit;
			break;
		}
		case _Key_Switch_3:
		{
...
bool PCEchoProcess(u8 *pMsg)
{
	...
	switch (u8Cmd)
	{
		...
		case 0x48:
		{
			...
			else if ((u8Array == 0) && ((u8Led <= 0x19) && (u8Led >= 0x0E)))
			{
				const u16 u16LedTrick[] = 
				{
					...
					_Led_Trick_Shortcut_2,
					_Led_Trick_Shortcut_3,
					_Led_Trick_Shortcut_4,
					_Led_Trick_Shortcut_5,
					_Led_Trick_Shortcut_6,
					_Led_Trick_Shortcut_7,
					_Led_Trick_Shortcut_8,
				};
				u8Led -= 0x0E;
